module.exports = app => {
  ...
  router.get('/news/init', async (req, res) => {
    ...
    await Article.deleteMany({})//慎用 清空article列表 传递一个空对象表示以任意条件去删除
    await Article.insertMany(newsList)//向articl插入数据
    res.send(newsList)
  })
  ...
  router.get('/news/list', async (req, res) => {
    ...
    //aggregate聚合查询 在一次查询里面同时执行多次查询  此方法效率更高
    const cats = await Category.aggregate([
      { $match: { parent: parent._id } },//执行一个match操作 条件查询  过滤
      {
        $lookup: {
          from: 'articles',//关联查询，关联哪个集合  集合名默认是模型的首字母小写复数形式
          localField: "_id",//本地键
  ...
  router.get('/heroes/list', async (req, res) => {
    ...
    const cats = await Category.aggregate([
      ...
      {
        $lookup: {
          from: 'heroes',//关联查询，关联哪个集合  集合名默认是模型的首字母小写复数形式
          localField: "_id",//本地字段
          foreignField: "categories",//外地字段
          as: 'heroList'//起个名字
        }
      }
    ])
